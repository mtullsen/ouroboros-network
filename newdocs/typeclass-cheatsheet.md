...
(view fixed-font, at least 140 chars wide, see Diagramatic Conventions)
...
Looking at the arrow directions, it should be clear that `b :: B` determines all
the other types.

### Type Constructors That are Type-Generic

Type families are being applied in the definitions of these, thus effectively look like
a type-family.  This list is not exhaustive.
...
```
Classes for headers and blocks:
```haskell
...
## Key Base Types for Quick Reference
### ouroboros-consensus:
...
- NOTE:  When convenient, implicit conversions are being made between "type variables"
  and "type-functions applied to variables". E.g.,
  ```
       p  <==> BlockProtocol b
       cds <==> ChainDepState p
  ```
...
- To reduce the "noise", these type-class constraints are being ignored:
  `NoThunks`, `Eq`, `Show`, `HasCallStack`; but `Ord` is not being ignored.
