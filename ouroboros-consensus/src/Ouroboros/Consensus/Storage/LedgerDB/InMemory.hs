...
--    ifT b msg what = if (blockSlot b > 4536020) then TRACE.trace msg what else what
    ...
    withHydratedLedgerState urs f b aks =
      (\rs -> f $ withLedgerTables (ledgerDbCurrent db) -- (if blockSlot b > 4536020 then mapLedgerTables (\tbs@(ApplyValuesMK (UtxoValues vals)) -> TRACE.trace (show $ Map.size $ vals) tbs) rs else
                                                           rs)--)
